...
- **核心机制示意图**：每张卡在「结构化解释」里配 1 张内联 SVG 图（Agent 决策循环 / 上下文位置效应 Lost-in-the-Middle / Skill 结构），不依赖 CDN、断网也能显示
- **💡 通俗理解**：用生活化比喻说清概念（AI 辅助整理 · 本人已核查），放在 60 秒速读之后、学习目标之前
...
按 SKILL.md 第 6 节定义的 13 条自检规则，对本次生成的 4 张概念卡做了**逐条自动化核查**（脚本：`audit.py`），结果：**31 / 31 子项全部通过**。

- [x] **1. 文件落在 `learning-materials/<slug>.html`，文件名和概念一致** — `agent.html` / `llm-context.html` / `skill.html` / `concept-relationship.html` 4 个文件齐全
- [x] **2. HTML 含 11 个区块**（banner / 通俗理解 / 速读 / 目标 / 问题 / 解释 / 核心机制图 / 案例 / 辨析 / 互动 / 来源 + 人工核查） — 3 张概念卡区块结构完整，audit 全部检测到
- [x] **3. 60 秒速读 ≤ 120 字** — Agent 106 字 · 上下文 119 字 · Skill 114 字
- [x] **4. 至少 1 个真实应用场景**（不是泛泛的「可用于……」） — 每张卡都有具体故事，含人名/数字/出处
- [x] **5. 至少 2 个易混淆邻居概念 + 区分点** — 每张卡的概念辨析表 ≥ 2 行
- [x] **6. 「💡 通俗理解」段在 60 秒速读之后、学习目标之前；去第一人称 + 一个生活化比喻；≤ 150 字；正文不加粗；字体统一；并注明「AI 辅助整理·本人已核查」** — Agent「点外卖」· 上下文「记性不好的人打电话」· Skill「老员工的经验手册」· 各卡均 15px/行高 1.7/正文色，无 `<strong>` 残留
- [x] **7. 每张卡含 1 张「核心机制示意图」**（内联 SVG、不依赖 CDN、断网可显示） — Agent「决策循环」· 上下文「位置效应 Lost-in-the-Middle」· Skill「SKILL.md 结构」
- [x] **8. 所有参考来源 URL 可访问** — 来源 100% 来自 OpenAI / Anthropic / WorkBuddy / Chroma / ACL Anthology / Wikipedia 等官方或可核查站点，未伪造
- [x] **9. HTML 能在断网环境打开**（除 Mermaid CDN 外不依赖外网） — 所有 CSS / JS / 内联 SVG；仅 Mermaid 用 CDN，断网时自动降级为文字描述
- [x] **10. 互动自测 8 题：答对升层 / 答错降层；答错立即给答案 + 解释 + 来源锚点** — L0-L3 各 2 题；JS 状态机 `0≤L≤3`；答错反馈含 `📎 来源 → #ref-N` 锚点
- [x] **11. 参考来源每条都被正文角标引用** — 5 条来源 ref-1~ref-5 全部在「结构化解释 / 应用案例 / 概念辨析」中被 `[N]` 引用
- [x] **12. 至少 1 道开放题**（不可自动评分） — Agent Q8「设计客服 Agent」· 上下文 Q8「5 步上下文审计」· Skill Q8「设计 Skill 仓库结构」
- [x] **13. 「人工核查记录」meta-card 在卡片底部** — 每张卡底部都有，写明核查了什么、改了什么

**完整审计报告**：[`notes/skill-run-2026-09-08.md`](notes/skill-run-2026-09-08.md) — 含 13 条勾选镜像、修复记录、commit 引用、autoxia 自动化脚本入口。
